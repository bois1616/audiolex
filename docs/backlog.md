...
- [ ] [P1] [Audio] [→Sonnet] `WavFile.decode` gegen korrupte Eingaben absichern: eine negative oder über das Dateiende hinauslaufende Chunk-Größe führt aktuell zu einer Endlosschleife bzw. unklarem Indexfehler statt einer `require`-Meldung (Review-Kleinbefund 2026-07-09). Guard + Unit-Tests mit kaputten Fixtures. Relevanz: Der Fehlerzustand-AC des Lernmodus-Screens („Meldung statt Absturz") setzt voraus, dass `decode` bei korrupter Datei schnell und benannt fehlschlägt statt zu hängen.
