...
- **Nutzerdaten entstehen, die nicht im Repo liegen.** Anders als die gitignorierten Korpus-WAVs sind eigene Aufnahmen unwiederbringlich, wenn das Gerät verloren geht oder die App deinstalliert wird. Eine Sicherung ist **nicht** Teil dieses ADRs und bleibt eine offene Frage. — **Beantwortet am 2026-08-06 durch ADR-0013:** Export als ZIP ins Dokumentenverzeichnis, vom Nutzer ausgelöst, plus Import zum Zurückspielen. Dabei kam heraus, dass die Aufnahmen über Androids `allowBackup` bereits ungefragt ins Google-Konto wanderten — was der Datenschutz-Zusage dieser App widersprach und mit derselben Version abgeschaltet wird.
...
## Nachtrag 2026-08-06: Die Quellentrennung wird durch Kontingente ersetzt

Entscheidung 1 und Punkt 5 (dritter Spiegelstrich) beschreiben den Eigen-Korpus als **zweite Quelle** neben dem mitgelieferten. Batch C hat das so umgesetzt: eine Einstellung `CorpusSource { MITGELIEFERT, EIGENE, BEIDE }`. Batch D nimmt das wieder zurück — auf Einwand des Autors, und er hat recht.

**Der Befund:** Der mitgelieferte Korpus hat genau **eine** Stimme (`thorsten`, alle 68 Aufnahmen). Eine Wahl „mitgeliefert / eigene / beide" ist deshalb nichts anderes als eine Sprecherauswahl mit fest verdrahteten Gruppen. Zwei Einstellungen beantworten dieselbe Frage — „woraus trainiere ich heute?" — und die Naht zwischen ihnen wird mit jedem weiteren Sprecher unangenehmer, nicht harmloser. Autor am 2026-08-06: „Der mitgelieferte Korpus muss gegenüber den anderen eigentlich nicht besonders hervorgehoben werden. Eigentlich kann die Unterscheidung Eigene/Mitgelieferte auf eine Korpusauswahl reduziert werden."

**Die Entscheidung:** `CorpusSource` entfällt ersatzlos. An seine Stelle tritt eine Mehrfachauswahl über die tatsächlich vorhandenen Sprecher-Kontingente, `thorsten` als eines darunter — ohne Sonderstatus. „Mitgeliefert" ist dann `thorsten`, „beide" ist alles, „nur eigene" ist alles außer `thorsten`. Die letzte Zeile ist der Grund für **Mehrfach**auswahl: mit Einfachauswahl ließe sich „alle eigenen" nicht ausdrücken.

**Gespeichert wird die Abwahl, nicht die Auswahl.** Damit ist „alle" die Vorbelegung, ohne dass irgendwo Namen stehen müssen, und ein neu hinzukommendes Kontingent ist automatisch dabei, statt stumm zu bleiben, bis es jemand anhakt. Das war der einzige echte Verlust gegenüber der offenen Kategorie „Eigene" — er entfällt durch diese Speicherrichtung. Nötig wurde sie ohnehin durch die vom Autor gewünschte Schaltfläche „alle abwählen": Bei gespeicherter Auswahl müsste „leer" gleichzeitig „alles" bedeuten, und die Schaltfläche täte sichtbar nichts.

**Punkt 5, dritter Spiegelstrich, gilt weiterhin — genauer sogar.** Die Schadensbegrenzung hing nie an der Zweiteilung, sondern daran, dass sich das Ungeprüfte vom Kuratierten trennen lässt. Über Kontingente geht das **feiner**: Ein einzelner unsauber eingesprochener Sprecher lässt sich isolieren, statt „alle eigenen" pauschal abzuschalten. Der kuratierte Korpus wird nach wie vor nicht verunreinigt — er ist nur kein Sonderfall mehr, sondern das Kontingent `thorsten`.

**Was der Nachtrag mitbringt und was er auslöst:**

- Ein **Index je Kontingent** (Anzahl trainierbarer Wörter und Sätze) macht vor der Auswahl sichtbar, was ein Durchlauf enthalten wird. Ein Kontingent ohne trainierbare Einträge kann gar nicht erst in der Liste stehen, weil sie aus den Einträgen abgeleitet ist.
- Das Sprecher-Feld wird von **Beschriftung zu Struktur**. Damit wird ein Tippfehler teuer: „Stephan" und „Stepahn" stehen ab hier dauerhaft als zwei Kontingente nebeneinander, und Batch B kann den Sprecher eines Eintrags **nicht** ändern (nur Text, Aufnahme, Löschen). Batch D beugt an der Wurzel vor, indem die Aufnahmemaske vorhandene Namen vorschlägt; die Heilung (umbenennen, einzelnen Eintrag umhängen, Kontingent löschen) ist ein eigenes Backlog-Item. Autor-Hinweis, der darauf führte: „Problem sehe ich nur bei der Verwaltung mehrerer Korpus, Änderung, Erweiterung."
- Der Rückbau kostet einen **Tabellenumbau** in der Migration (SQLite kann Spalten nicht entfernen) — der erste echte Härtetest der Migrationsinfrastruktur aus Batch C, und der Grund, ihn wieder direkt an der Datenbank zu prüfen statt am Bildschirm.
