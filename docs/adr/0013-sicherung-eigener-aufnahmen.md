...
## Nachtrag 2026-08-07 (zweiter): Eigene Störgeräusche gehören in die Sicherung

**Anlass:** Das M4-Item „Eigene Störgeräusche aufnehmen, importieren und löschen" bringt die dritte Sorte selbsterzeugter, unwiederbringlicher Inhalte (Autor-Anforderung im Item). ADR-0014 macht sie zusätzlich zum Veröffentlichungsweg: Eine Ausgabe der App enthält keine gebündelten Störgeräusche mehr — was der Nutzer an Geräuschen hat, hat er selbst aufgenommen oder importiert. Ein Verlust wäre dort kein Ärgernis, sondern der Verlust des gesamten Inhalts dieser App-Sektion.

**Korrigierte Entscheidung 6 (zweite Stufe):** Gesichert wird, was sich nicht neu bildet — der Eigen-Korpus, die Sitzungshistorie und **eigene Störgeräusche**. Der gebündelte Katalog bleibt draußen: Er liegt im Repository bzw. wird zur Build-Zeit erzeugt (ADR-0014) und ist damit das einzige Geräusch-Inventar, das sich wiederherstellen lässt.

**Format — der dritte Ordner im Archiv:** Der Aufbau „ein Ordner je Inhaltsart" trägt die Erweiterung ohne Formatbruch: Zu `eigene-aufnahmen/` und `sitzungen/` kommt `stoergeraeusche/` mit `geraeusche.json` und den WAVs. Rein additiv: Die Formatmarke bleibt `format = 1`, ein Archiv **ohne** den Ordner bleibt gültig (alle Sicherungen, die vor dieser Änderung entstanden sind, haben keinen; fehlender Ordner → leere Liste). Die Unterscheidung aus dem ersten Nachtrag gilt entsprechend: *fehlende* Geräusche sind normal, *vorhandene aber beschädigte* Metadaten machen das ganze Archiv unlesbar — ein kaputter Bestand darf nicht als „0 Geräusche übernommen" durchgehen.

**Zusammenführen wie bei den Aufnahmen:** Identität ist die id, kollisionsfrei erzeugt; unbekannt kommt hinzu, bekannt wird übersprungen, nichts wird überschrieben oder gelöscht (Entscheidung 5 gilt entsprechend). Fehlt nach einem Import das Geräusch, das die Einstellung `noiseScenario` ausgewählt hat, greift die bestehende Fallback-Auflösung aus ADR-0010 — der Import muss keinen Auswahlzustand mittragen.
